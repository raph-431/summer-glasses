...
**Contract**: `contract/` is a Foundry project (deps gitignored — `cd contract && forge install` after a fresh clone). `SummerGlasses.sol`: ERC-721 with gift-code lazy minting — `gift(claimAddr)` escrows payment against an ephemeral claim key's address; `redeem(claimAddr, to, sig)` mints when the claim key has signed `redeemDigest(to)` (raw digest, no EIP-191 prefix — signature binds the recipient, so redeems can't be front-run and anyone can relay; msg.sender gets the gas stipend); `reclaim` refunds the gifter after 365 days. Seed fixed at redemption from prevrandao+tokenId+recipient. Artwork stored via `SSTORE2.sol` chunks + three wrapper parts (from `dist/art-*.txt`); `tokenURI` reassembles the bootstrap with the token's seed, fully on-chain. `forge test` (28 tests) includes a round-trip that writes `dist/.onchain-token.html` for browser checks. Deploy: `script/Deploy.s.sol` (env: PRICE/STIPEND/MAX_SUPPLY); freeze art manually at launch. Testing gotcha: helpers that call the contract (even views) consume a pending `vm.prank`/`vm.expectRevert` — precompute signatures first.
