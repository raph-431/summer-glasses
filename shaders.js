...
vec3 bubbleAt(vec2 s){
  ...
  vec3 bb = bubbleLayer(s, cs2*2.6, 3.1, 1.18);
  vec3 b2 = bubbleLayer(s + 13.7, cs2, 17.0, 0.92);
  vec3 b3 = bubbleLayer(s + 41.3, cs2*0.45, 29.0, 0.78);
  // second mid + dust families on their own grids: ~3x the population
  vec3 b4 = bubbleLayer(s + 27.9, cs2*1.15, 7.7, 0.92);
  vec3 b5 = bubbleLayer(s + 55.1, cs2*0.52, 47.0, 0.78);
  if(b2.x > bb.x) bb = b2;
  if(b3.x > bb.x) bb = b3;
  if(b4.x > bb.x) bb = b4;
  if(b5.x > bb.x) bb = b5;
...
// misty film: strongest around the middle of the glass, patchy — high
// contrast between fog patches and clear glass, so an uncut glass still
// reads as polished, not frosted
float mistAmount(vec2 s){
  ...
  float blotch = 0.30 + 0.90*fbm(vec2(cos(thm), sin(thm))*1.6 + vec2(2.2, s.y*4.5));
  return 0.42 * band * blotch * u_cond;
}
...
// noise-free ice SDF for photon transport — same cubes as the render's
// detailed version, minus the surface lumps (the fracture jitter supplies
// the caustic irregularity instead)
float iceSDFs(vec3 p){
  float dm = 1e5;
  for(int i=0;i<3;i++){
    if(float(i) > u_iceN - 0.5) break;
    float fi = float(i);
    vec3 q = p - u_icePos[i];
    float c = cos(u_iceRot[i]), s2 = sin(u_iceRot[i]);
    q.xz = mat2(c, -s2, s2, c)*q.xz;
    vec3 asp = vec3(0.80) + 0.40*vec3(hash12(vec2(fi, 1.3)),
                                      hash12(vec2(fi, 4.7)),
                                      hash12(vec2(fi, 8.1)));
    vec3 b = abs(q) - u_iceR[i]*0.95*asp;
    dm = min(dm, length(max(b, vec3(0.0))) + min(max(b.x, max(b.y, b.z)), 0.0) - u_iceR[i]*0.09);
  }
  return dm;
}
vec3 iceNormalS(vec3 p){
  const vec2 e = vec2(0.005, 0.0);
  return normalize(vec3(
    iceSDFs(p+e.xyy) - iceSDFs(p-e.xyy),
    iceSDFs(p+e.yxy) - iceSDFs(p-e.yxy),
    iceSDFs(p+e.yyx) - iceSDFs(p-e.yyx)));
}
float iceMarchS(vec3 o, vec3 dir, float tmax){
  float t2 = 0.01;
  for(int i=0;i<12;i++){
    if(t2 > tmax) return -1.0;
    float d = iceSDFs(o + dir*t2);
    if(d < 0.005) return t2;
    t2 += max(d*0.9, 0.008);
  }
  return -1.0;
}
...
uniform float u_canRot;         // per-deal spin of the palm around that anchor
...
vec2 canopyField(vec2 cp, vec2 off){
  ...
  if(u_canopy < 1.5){
    // fine-leafed tree (olive, acacia): small busy clumps, many little
    // gaps. Gaps this small are all penumbra, so the mask grades softly
    // and the three lights' dapple pools blend instead of stippling.
    float n1 = fbm(cp*4.4 + off*1.3);
    float n2 = fbm(cp*9.0 - off*1.8 + 9.2);
    float m = smoothstep(0.32, 0.70, n1*0.55 + n2*0.52);
    return vec2(m, n2);
  }
  if(u_canopy < 2.5){
    ...
    float ca2 = cos(u_canRot), sa2 = sin(u_canRot);
    vec2 qq = mat2(ca2, -sa2, sa2, ca2)*(cp - u_canopyC - off*0.3);
    vec2 q = (qq - vec2(4.4, 3.0))/6.2;
    float rr2 = length(q);
  ...
  if(u_canopy < 3.5){
    ...
    // the slat direction spins per deal
    float cpc = cos(u_canRot), cps = sin(u_canRot);
    vec2 rp = mat2(cpc, -cps, cps, cpc)*cp;
    float sl = 0.5 + 0.5*sin((rp.x + rp.y*0.15)*7.0);
...
void main(){
  ...
  vec3 tint = vec3(1.0);
  if(u_iceN > 0.5){
    // TRUE refraction through an ice cube on the path: in at the near
    // face, a short march across, out the far face. Above the liquid line
    // the air/ice contrast bends hard; inside the liquid it's gentle.
    float tI = iceMarchS(P2, d, s);
    if(tI > 0.0){
      if(inLiq) tint *= exp(-tI * u_liqSig * 0.45);   // liquid up to the ice
      vec3 Q1 = P2 + d*tI;
      vec3 Ni1 = iceNormalS(Q1);
      vec3 dI = refract(d, Ni1, nIn/1.31);
      if(dot(dI,dI) < 0.1) dI = reflect(d, Ni1);
      vec3 Q2 = Q1;
      for(int k=0;k<8;k++){
        Q2 += dI*0.035;
        if(iceSDFs(Q2) > 0.005) break;
      }
      vec3 Ni2 = iceNormalS(Q2);
      vec3 dO = refract(dI, -Ni2, 1.31/nIn);
      if(dot(dO,dO) < 0.1) dO = reflect(dI, -Ni2);
      // internal fractures scatter a touch of the beam
      vec2 jj2 = hash22(uv*613.0 + u_seed*7.7) - 0.5;
      d = normalize(dO + vec3(jj2.x, 0.0, jj2.y)*0.05);
      tint *= vec3(0.90, 0.95, 1.0);
      P2 = Q2;
      s = innerChord(P2, d);
      if(s <= 0.0){ kill(); return; }
    }
...
float iceSDF(vec3 p){
  ...
  for(int i=0;i<3;i++){
    ...
    vec3 asp = vec3(0.80) + 0.40*vec3(hash12(vec2(fi, 1.3)),
                                      hash12(vec2(fi, 4.7)),
                                      hash12(vec2(fi, 8.1)));
    // mostly-flat faces with just a small edge chamfer — tray ice, not pebbles
    vec3 b = abs(q) - u_iceR[i]*0.95*asp;
    float dd = length(max(b, vec3(0.0))) + min(max(b.x, max(b.y, b.z)), 0.0) - u_iceR[i]*0.09;
...
void main(){
  ...
  // ice shouldering above the rim, seen directly in open air (through-wall
  // and through-surface views are handled inside the glass branches)
  float tIceAir = -1.0;
  if(u_iceN > 0.5){
    float a2 = dot(rd.xz, rd.xz);
    float b2 = 2.0*dot(ro.xz, rd.xz);
    float c2 = dot(ro.xz, ro.xz) - (u_maxR + 0.1)*(u_maxR + 0.1);
    float d2c = b2*b2 - 4.0*a2*c2;
    if(d2c > 0.0 && a2 > 1e-6){
      float sA = max((-b2 - sqrt(d2c))/(2.0*a2), 0.0);
      float sB = (-b2 + sqrt(d2c))/(2.0*a2);
      float tI = iceMarch(ro + rd*sA, rd, sB - sA);
      if(tI > 0.0){
        vec3 Q = ro + rd*(sA + tI);
  ...
  if(tIceAir > 0.0 && tIceAir < min(tTable, tLiq)){
    // ---------- bare ice above the rim ----------
    hitT = tIceAir;
    col = shadeIce(ro + rd*tIceAir, rd);
  } else if(tGlass > 0.0 && tGlass < min(tTable, tLiq)){
    // ---------- the glass itself ----------
    hitT = tGlass;
    vec3 P = ro + rd*tGlass;
    vec3 N = glassNormal(P);
    ...
    // fade the far table into the backdrop; its exposure rides the sun slider
    col = mix(col, u_backCol*0.96*(0.55 + 0.45*u_sun), smoothstep(4.0, 12.0, tTable));
  }else{
    // the backdrop is the environment, overexposed in proportion to the sun
    col = mix(envColor(rd), u_backCol, 0.45)*(0.55 + 0.45*u_sun);
  }
