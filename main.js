...
function buildAudio(){
  ...
  const subG = ctx.createGain(); subG.gain.value = 0.045; subG.connect(xtal);
...
function frame(){
  ...
  if(lightPaint && beamOn){
    ...
    if(beamEnv > 0.004){
      const az = beamAz0 + 0.9*snz(t*0.165*beamSpd, beamPh1);
      const el = Math.min(Math.max(
        beamEl0 + 0.28*snz(t*0.135*beamSpd, beamPh2), 0.22), 1.05);
      ...
      const aim = [
        maxR*(0.30*snz(t*0.285*beamSpd, beamPh3) + 0.05*snz(t*1.35, beamPh3 + 9.1)),
        shape.H*(0.55 + 0.30*snz(t*0.210*beamSpd, beamPh3 + 31.7)),
        maxR*(0.30*snz(t*0.315*beamSpd, beamPh3 + 57.1) + 0.05*snz(t*1.50, beamPh3 + 4.3))];
      ...
      const roll = beamAz0*1.7 + 0.4*snz(t*0.090*beamSpd, beamPh1 + 7.7);
